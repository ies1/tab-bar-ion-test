...
import { Midata, Resource, Patient} from 'Midata';
...
export class MidataService {
  ...
  private midata: Midata;
    ...
    constructor(
      ...
      this.midata = new Midata('https://test.midata.coop', 'MiTrendS-Tablet');
      //this.midata = new Midata('https://demo.midata.coop', 'MiTrendS-Tablet');
      //this.midata.setUserLanguage(this.languageService.getSuitableLanguage(this.languageService.language));
    ...
    if(this.getNetworkState()) {
      return this.getConnection().fetchFHIRConformanceStatement()
        ...
        .then((rsp : AuthAndPatResponse) => {
          let setPat = this.setSecureStoragePatient(rsp.patientResource);
          let setToken = this.setSecureStorageToken(rsp.authResponse.refresh_token);

          return Promise.all([setPat, setToken]);
        })
        .then(() => {
  ...
  /**
   * Stores a given patient inside the secure storage
   * @param patient The patient to be stored
   * @returns Promise of type any
   */
  private setSecureStoragePatient(patient: Patient): Promise<any> {
    return new Promise<String>((resolve, reject) =>{
      return this.secureStorage.create('MIDATA_CONNECTION_SERVICE')
        .then((storage: SecureStorageObject) => {
          return storage.set('patient', JSON.stringify(patient));
        })
        .then((data) => {
          return resolve(data);
        }).catch((error) => {
  ...
  /**
   * Gets the patient held inside the secure storage
   * @returns Promise of type any
   */
  getSecureStoragePatient(): Promise<Patient> {
    return new Promise<Patient>((resolve, reject) =>{
      return this.secureStorage.create('MIDATA_CONNECTION_SERVICE')
            .then((storage: SecureStorageObject) => {
              return storage.get('patient')
            })
            .then((data) => {
              let jsonData = JSON.parse(data);
              let pat = new Patient("unknown");
              Object.assign(pat, jsonData)
              return resolve(pat);
            }).catch((error) => {
              return reject(error);
            })
